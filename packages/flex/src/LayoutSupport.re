...
module Create = (Node: Spec.Node, Encoding: Spec.Encoding) => {
  ...
  let computedEdgeValuePosition = (style, edge, defaultValue) =>
    ...
    | _ =>
      raise(
        Invalid_argument("Should never be called with a non-concrete edge"),
      )
    };
  let computedEdgeValueMarginOriginal = (style, edge, defaultValue) => {
    ...
    if (!isUndefined(forEdge)) {
      forEdge;
               ...
               && !isUndefined(style.marginVertical)) {
  ...
  let computedEdgeValueBorderOriginal = (style, edge, defaultValue) => {
    ...
    if (!isUndefined(forEdge)) {
      forEdge;
               ...
               && !isUndefined(style.borderVertical)) {
  ...
  let computedEdgeValuePaddingOriginal = (style, edge, defaultValue) => {
    ...
    if (!isUndefined(forEdge)) {
      forEdge;
               ...
               && !isUndefined(style.paddingVertical)) {
   ...
   */
  let dummyMeasure =
      (_context, _width, _widthMeasureMode, _height, _heightMeasureMode) =>
    if (failOnDummyMeasure) {
      raise(
        Invalid_argument(
          "A node does not have its measure function "
          ++ " implemented, yet requires measurement",
        ),
      );
    } else {
      {
        LayoutTypes.width: zero,
        height: zero,
      };
    };
  /*
  ...
  let getLeadingPadding = (node, axis) =>
    if (isRowDirection(axis)
      ...
      fmaxf(
        computedEdgeValuePadding(node.style, leadingEdgeForAxis(axis), zero),
        zero,
      );
    };
  let getTrailingPadding = (node, axis) =>
    if (isRowDirection(axis)
      ...
      fmaxf(
        computedEdgeValuePadding(
          node.style,
          trailingEdgeForAxis(axis),
          zero,
        ),
        zero,
      );
  ...
  let getLeadingBorder = (node, axis) =>
    if (isRowDirection(axis)
      ...
      fmaxf(
        computedEdgeValueBorder(node.style, leadingEdgeForAxis(axis), zero),
        zero,
      );
    };
  let getTrailingBorder = (node, axis) =>
    if (isRowDirection(axis)
        && !isUndefined(node.style.borderEnd)
        && node.style.borderEnd >= zero) {
      node.style.borderEnd;
    } else {
      fmaxf(
        computedEdgeValueBorder(node.style, trailingEdgeForAxis(axis), zero),
        zero,
      );
    };
  ...
  let isLeadingPosDefined = (node, axis) =>
    ...
    && !
         isUndefined(
           computedEdgeValuePosition(node.style, Start, cssUndefined),
         )
    || !
         isUndefined(
           computedEdgeValuePosition(
             node.style,
             leadingEdgeForAxis(axis),
             cssUndefined,
           ),
         );
  let isTrailingPosDefined = (node, axis) =>
    ...
    && !
         isUndefined(
           computedEdgeValuePosition(node.style, End, cssUndefined),
         )
    || !
         isUndefined(
           computedEdgeValuePosition(
